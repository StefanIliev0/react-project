...
export function ProfileDiv({ to, postOwner, imgUrl, nickname, removeUser, unapproveCandidate, approveCandidate }) {
    const [isHovered, setIsHovered] = useState(false);
    const [remove, setRemove] = useState(false);


    function removeUserFromObject(e) {
        if (unapproveCandidate) {
            unapproveCandidate(to)
        } else {
            removeUser(to);
        }
    }
        ...
        <div className={styles["container-div"]} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            {remove ? (
                <div>
                    ...
                    <div>
                        <button className={styles["removed-btn"]} onClick={removeUserFromObject}>Yes!</button>
                ...
                <>
                    <div className={styles["buttons-div"]}>
                        ...
                        {(postOwner && isHovered) ? (<>
                            ...
                            {approveCandidate && <button className={styles["removed-btn"]} onClick={() => approveCandidate(to)}>Approve</button>}
                        </>
                        ) : null}
                    </div>
