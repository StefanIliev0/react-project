import { addCandidate, create, giveMeAllCandidates, giveMeAllMembers ,removeUser ,update ,giveMeOwner, giveMeMembers, giveMe , giveMeCandidates, giveMeAll, approveUser, unapproveUser} from "./requester";
...
export async function getGroupDeatails (groupId){
    ...
    const owner = await giveMeOwner("group" , atcId) ;
    const members = await giveMeMembers("group" , atcId) ; 
    const candidates = await giveMeCandidates("group" , atcId); 
    const groupActivities = await giveMeAll("activity" , groupId)
